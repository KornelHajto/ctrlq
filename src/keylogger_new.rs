...
impl KeyLogger {
    ...
    pub fn start_logging(&mut self) -> Result<(mpsc::Receiver<KeyStats>, watch::Sender<bool>)> {
        ...
        thread::spawn(move || {
            if let Err(e) = Self::logging_loop(device_path, &mut stats, data_file, stats_tx, shutdown_rx) {
                eprintln!("Logging error: {}", e);
            }
        });

        Ok((stats_rx, shutdown_tx))
    }
    ...
    fn logging_loop(
        ...
        loop {
            ...
            match device.fetch_events() {
                Ok(events) => {
                    for event in events {
                        if let InputEvent::Key(key_event) = event.kind() {
